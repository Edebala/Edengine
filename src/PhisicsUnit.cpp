...
short Creature::Wall_Collision(bool Horiz,int dist, short block){
	if(Horiz){
		if(speed_x > 0)
    ...
    pos_y = int(pos_y+dist+speed_y-int(speed_y)+mysize/2)-mysize/2;
    Standing = true;
  }
  else pos_y = int(pos_y + dist + speed_y) + mysize / 2;
  Stop(Horiz);
  if (block >= LAVA)
    return 1;
  return 0;
